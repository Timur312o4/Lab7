...
public class CollectionManager {
    private CopyOnWriteArrayList<City> cityCollection;
    private ZonedDateTime creationDate;
    /**
     * Конструктор класса CollectionManager
     */
    public CollectionManager(){
        cityCollection=new CopyOnWriteArrayList<City>();
     ...
     */
    public CollectionManager(CopyOnWriteArrayList<City> collection){
        ...
        if (!cityCollection.isEmpty()){
            try{
                ...
                cityCollection = this.getCityCollection().stream()
                        .filter(city -> !city.getId().equals(id)).collect(Collectors.toCollection(CopyOnWriteArrayList::new));
    ...
    */
     ...
     */
    public CopyOnWriteArrayList<City> getCityCollection(){
     ...
     * @param collection коллекция городов
     */
    public void setCityCollection(CopyOnWriteArrayList<City> collection){
     ...
     * @return возвращает результат операции
     */
    public boolean clearCollection(User user){
        CopyOnWriteArrayList<City> citiesUser = this.getElementsByUser(user);
        cityCollection.removeAll(citiesUser);
        return true;
    }
    public CopyOnWriteArrayList<City> getElementsByUser(User user){
        CopyOnWriteArrayList<City> elementsByUser = new CopyOnWriteArrayList<>();
        for (City city : cityCollection){
            if(city.getUserName().equals(user.getName())){
                elementsByUser.add(city);
            }
        }
        return elementsByUser;
    }
