 ...
 * @author Timur
 */
//Мб и этот класс особо не нужен ну ладно
public class ValidatorParse { //Ещё раз располовинить или же и так норм, надо спросить. Можно в принципе взять все валидаторы и закинуть в parse валидатор и по идее должно быть норм
    ...
    /*public static boolean validatepars(String line){
        ...
        if (line.split(",").length!=16 && line.split(";").length!=15){              // второй способ, if((parts[11] == null && parts.length()!=12) || (parts[11] != null && parts.length()!=15)) return false
        ...
        try{
            Date creationDate = Parser.parseDatefromFile(parts[4].trim());
