...
public class InsertAt extends Command{
     ...
     */ // жесть как это реализовывать, если только отдельный столбик добавить, который отвечает за индексацию по коллекции
    // ну зато после реализации этой команды работы, то не осталось считай, просто надо удостовериться, что все корректно работает
    @Override
