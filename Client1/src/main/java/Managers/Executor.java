...
public class Executor {
    ...
    public void userMode(){
        ...
        while (true){
            ...
            try {
                ...
                if (this.user == null){
                    System.out.println("Здравствуйте, вам нужно пройти авторизацию.");
                    this.user = this.clientAuthorization();
                    System.out.println("Авторизация завершена успешно.");
                    continue;
                }
                Request request = new Request(this.user,command[0].trim(), command[1].trim());
                Response response = client.sendAnsOnRequest(request);
                if (response.getTypeResponse().equalsIgnoreCase("GENERATE_CITY")) {
                    City city = GeneratorCity.generate(this.getUsedScanner(),this.user.getName());
                    request = new Request(this.user,command[0].trim(), command[1].trim(), city);
                    Response newResponse = client.sendAnsOnRequest(request); // получаем ответ на наш запрос
                    if (response.getTypeResponse().equalsIgnoreCase("ERROR")){
    ...
    public void scriptMode(String fileName) {
        ...
        try {
            ...
            while (checkCountFileinFilesList(fileNamesList,fileName)&&newScanner.hasNext()){
                ...
                Request request = new Request(user,scriptCommand[0].trim(), scriptCommand[1].trim());
                ...
                if (response.getTypeResponse().equalsIgnoreCase("GENERATE_CITY")){
                    City city = GeneratorCity.generate(this.getUsedScanner(),user.getName());// так не опасно ли делать
                    request = new Request(user, scriptCommand[0].trim(),scriptCommand[1].trim(),city);
                    Response newResponse = client.sendAnsOnRequest(request); // получаем ответ на наш запрос
                    Console.println(newResponse.getResponse());
    ...
    public User clientAuthorization(){
        System.out.println("Для работы с коллекцией на сервере нужно пройти авторизацию.");
        Response registerResponse;
        try{
            System.out.println("Вы уже зарегистрированы в системе? Если да введите yes, иначе no.");
            while(true){
                String ansClient = usedScanner.nextLine();
                if (ansClient.equalsIgnoreCase("yes")) {
                    System.out.println("Введите ваш логин");
                    String login1 = usedScanner.nextLine();
                    System.out.println("Введите ваш пароль");
                    String password1 = usedScanner.nextLine();
                    User user1 = new User(login1,password1);
                    Request authRequest = new Request(user1, "authorization","");
                    Response authResponse = client.sendAnsOnRequest(authRequest);
                    if(authResponse.getTypeResponse().equalsIgnoreCase("complete")){
                        System.out.println("Вы успешно авторизовались!");
                        return user1;
                    }else if (!authResponse.getTypeResponse().equalsIgnoreCase("Exit")){
                        System.out.println(authResponse.getResponse());
                        System.out.println("Возникли ошибки. Повторите попытку регистрации.");
                        System.out.println("Для регистрации введите no, для авторизации yes.");
                    }else{
                        System.exit(0);}
                }else if (ansClient.equalsIgnoreCase("no")){
                    System.out.println("Введите ваш логин");
                    String login1 = usedScanner.nextLine();
                    System.out.println("Введите ваш пароль");
                    String password1 = usedScanner.nextLine();
                    User user1 = new User(login1, password1);
                    Request register = new Request(user1, "register","");
                    registerResponse = this.client.sendAnsOnRequest(register);
                    if (registerResponse.getTypeResponse().equalsIgnoreCase("Complete")){;
                        System.out.println("Вы успешно зарегистрированы!");
                        return user1;
                    }else if (!registerResponse.getTypeResponse().equalsIgnoreCase("Exit")){
                        System.out.println(registerResponse.getResponse());
                        System.out.println("Возникли ошибки. Повторите попытку регистрации.");
                        System.out.println("Для регистрации введите no, для авторизации yes.");
                    }else{
                        System.exit(0);
